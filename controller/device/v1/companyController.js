 ...
 * @return {obj} : created Company. {status, message, data}
 */ 
const addCompany = async (req, res) => {
  try {
    ...
    let result = await dbService.createDocument(Company,data);
 ...
 * @return {obj} : found Company(s). {status, message, data}
 */
const findAllCompany = async (req,res) => {
  try {
    ...
    if (req.body.isCountOnly){
      let totalRecords = await dbService.countDocument(Company, query);
    ...
    let result = await dbService.getAllDocuments( Company,query,options);
...
const getCompanyCount = async (req,res) => {
  try {
    ...
    let result = await dbService.countDocument(Company,where);
 ...
 */
const getCompanyByAggregate = async (req,res)=>{
  try {
    let result = await dbService.getDocumentByAggregation(Company,req.body);
    if (result){
      return res.ok({ data :result });
    }
    return res.recordNotFound();
 ...
 * @param {obj} res : response contains updated documents of Company.
 * @return {obj} : number of deactivated documents of Company. {status, message, data}
 */
const softDeleteManyCompany = async (req,res) => {
  try {
    ...
    let result = await deleteDependentService.softDeleteCompany(query, updateBody);
    if (!result) {
      return res.recordNotFound();
    }
    return  res.ok({ data:result });
 ...
 * @return {obj} : created Companys. {status, message, data}
 */
const bulkInsertCompany = async (req,res)=>{
  try {
    ...
    let result = await dbService.bulkInsert(Company,data);
    return  res.ok({ data :result });
  } catch (error){
    if (error.name === 'ValidationError'){
      return res.validationError({ message : `Invalid Data, Validation Failed at ${ error.message}` });
 ...
 * @param {obj} res : response of updated Companys.
 * @return {obj} : updated Companys. {status, message, data}
 */
const bulkUpdateCompany = async (req,res)=>{
  try {
    ...
    if ( typeof req.body.data === 'object' && req.body.data !== null) {
      ...
      let result = await dbService.bulkUpdate(Company,filter,data);
      if (!result){
        return res.recordNotFound();
      }
      return  res.ok({ data :result });
...
const deleteManyCompany = async (req, res) => {
  try {
    ...
    if (req.body.isWarning) {
      let result = await deleteDependentService.countCompany(query);
      return res.ok({ data :result }); 
    }
    else {
      let result = await deleteDependentService.deleteCompany(query);
      return res.ok({ data :result });
    }
  } catch (error){
    return res.failureResponse({ data:error.message }); 
 ...
 * @param {obj} res : response contains updated document of Company.
 * @return {obj} : deactivated Company. {status, message, data}
 */
...
const partialUpdateCompany = async (req,res) => {
  try {
    ...
    let result = await dbService.findOneAndUpdateDocument(Company, query, data,{ new:true });
    if (!result) {
      return res.recordNotFound();
    }
    return res.ok({ data:result });
 ...
 * @return {obj} : found Company. {status, message, data}
 */
const getCompany = async (req,res) => {
  try {
    ...
    let result = await dbService.getSingleDocument(Company,query, options);
    if (result){
 ...
 * @return {obj} : deleted Company. {status, message, data}
 */
